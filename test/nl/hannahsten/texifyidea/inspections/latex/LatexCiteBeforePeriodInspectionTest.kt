...
class LatexCiteBeforePeriodInspectionTest : TexifyInspectionTestBase(LatexCiteBeforePeriodInspection()) {
    ...
    fun testWarning() {
        myFixture.configureByText(
            LatexFileType,
            """
            I cite.~<warning descr="\cite is placed after interpunction">\cite{</warning>knuth1990}
            However, e.g.~\cite{knuth1990} does not end a sentence, neither does Goossens et al.~\cite{goossens}.
            This e.g. is.~<warning descr="\cite is placed after interpunction">\cite{</warning>knuth1990}
            """.trimIndent()
        )
