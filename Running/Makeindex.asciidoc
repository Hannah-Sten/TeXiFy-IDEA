...
Note that when you use `imakeidx` but no `auxil/` or `out/` then imakeidx will handle makeindex itself. **However** if you use xindy then in order to allow imakeidx to run xindy, you need to provide the `-shell-escape` custom compiler argument in the run configuration.
