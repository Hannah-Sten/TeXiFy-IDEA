...
fun setUnicodeSupport(project: Project, enabled: Boolean = true) {
    ...
    if (enabled) {
        ...
        every { project.selectedRunConfig()?.let { it.getConfigOptions().compiler } } returns LualatexCompiler
    }
    else {
        // Unicode is not supported on pdflatex on texlive <= 2017.
        every { project.selectedRunConfig()?.let { it.getConfigOptions().compiler } } returns PdflatexCompiler
