...
class LatexPostfixExpressionSelector(private val mathOnly: Boolean = false, private val textOnly: Boolean = false) : PostfixTemplateExpressionSelector {
    ...
    override fun hasExpression(context: PsiElement, copyDocument: Document, newOffset: Int): Boolean = when {
    ...
    override fun getExpressions(context: PsiElement, document: Document, offset: Int): MutableList<PsiElement> = when (context.elementType) {
    ...
    fun getTopMostExpression(element: PsiElement): PsiElement = getExpressions(element, element.containingFile.fileDocument, element.textOffset).firstOrNull()
        ?: element
}